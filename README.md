...
| Lock only the `body`, or lock with `overflow: hidden`. | Either the root element or the `body` can be the scroll container depending on the theme, so both are locked – locking only one silently fails on the other kind of site. The value is `overflow: clip`, not `hidden`: `hidden` on the root element makes it a scroll container, which un-sticks any `position: sticky` header while the page is scrolled (the header drops off-screen by the scroll amount, taking an attached mega menu panel with it, and the frozen page can't scroll to reach it). `clip` locks identically but creates no scroll container, so a sticky header keeps sticking. |
