# Contributing to Kntnt Modal Mega Menu for Ollie

Thank you for considering a contribution. This plugin does one small thing – it makes Ollie Menu Designer's mega menu behave like a modal – and contributions of every size help, from a typo fix to a new capability.
...
- **Report a bug or request a feature.** [Open an issue](https://github.com/Kntnt/kntnt-modal-mega-menu-ollie/issues), and search the existing issues first to avoid duplicates.
- **Ask a question or float an idea.** Use [Discussions](https://github.com/Kntnt/kntnt-modal-mega-menu-ollie/discussions) rather than the issue tracker.
...
The plugin ships **behaviour, never style**. It makes the open menu modal – the page locked behind it, a tall menu scrolling inside itself – and touches nothing about how the menu looks. Colour, spacing, shadow, type: all of that stays with the user and with Ollie Menu Designer.

So a change that hard-codes a look, adds a settings page or otherwise makes a design decision on the user's behalf will not be merged. The whole point is that the plugin never guesses.

Likewise, it deliberately stays inside the gap Ollie Menu Designer leaves open. Rebuilding the mega menu itself – how it opens, positions or animates – belongs upstream in Ollie Menu Designer, not here. This plugin only adds the modal manners the block lacks.

Before you change anything, read [`AGENTS.md`](AGENTS.md) and [`agents.d/modal.md`](agents.d/modal.md). Both record the decisions that look like mistakes until you know why – chiefly that the menu's height is measured live rather than assumed, and that the internal-scroll script is a temporary workaround, not a permanent feature.
...
Click **Fork** at the top of [the repository](https://github.com/Kntnt/kntnt-modal-mega-menu-ollie) to get your own copy, then replace `YOUR-USERNAME` below:

```bash
git clone https://github.com/YOUR-USERNAME/kntnt-modal-mega-menu-ollie.git
cd kntnt-modal-mega-menu-ollie
composer install
```
...
To run your copy on a real site, symlink or copy the directory into `wp-content/plugins/` of a WordPress install running [Ollie Menu Designer](https://olliewp.com/), which provides the Dropdown Menu block the plugin extends.
...
Write the subject line in the imperative, as an instruction to the codebase – ‘Cap the mega menu to the viewport on desktop’, not ‘Fixed the menu’ or ‘menu fix’. Capitalise it, leave off the closing full stop and keep it to roughly 70 characters. This project does **not** use Conventional Commits: no `feat:`, no `fix(scope):`. Plain English reads better in a changelog and needs no translation to get there.

Name the change, not the file you touched. ‘Update modal.css’ tells the changelog nothing; ‘Lock the page while a mega menu is open’ tells it everything. Where the *why* is not obvious from the subject, put it in the body – a blank line, then prose wrapped at 72 columns. The body is where a workaround explains which upstream bug it is dodging.
...
- **Naming** follows the conventions in [`AGENTS.md`](AGENTS.md): namespace `Kntnt\Modal_Mega_Menu_Ollie`, slug and text domain `kntnt-modal-mega-menu-ollie`, and the `kntnt_modal_mega_menu_ollie_` prefix for anything in a global registry. The plugin defines no CSS classes of its own; its selectors reference Ollie Menu Designer's block classes, which are not the plugin's to prefix or rename.
...
One thing sits outside that freedom: the plugin's selectors depend on Ollie Menu Designer's block classes (`wp-block-ollie-mega-menu*`). Those are upstream's to change, not ours. If a Menu Designer release renames them, the fix is to follow the new names, not to work around them.
...
```bash
./build-release-zip.sh                       # → dist/kntnt-modal-mega-menu-ollie.zip
